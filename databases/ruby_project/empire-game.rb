...
methods/driver code:
  SETUP:
    -if not already created: make db, characters table, pause state table.
  START:
    -welcome player and ask what feature they'd like to access
    -run one of the following programs based on their response.

  CHARACTER CREATION:
    -driver code that uses gets.chomp and puts to ask questions where relevant and run the appropriate below methods to create a character
    -method that takes basic stats 
=end
